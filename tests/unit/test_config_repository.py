    ...
    def test_init_filter_sql_statements_execute(self, repo, db_session):
        ...
        for stmt in SQL_FILE.read_text(encoding="utf-8").split(";\n"):
            ...
            if stmt and "INSERT" in stmt.upper():
                if col_list in stmt:
                    # 预置规则 SQL 无 ORIGINAL_LANGUAGE 列（alembic 迁移后由 ORM 默认值/回填保证），
                    # 测试执行时在每个规则行 SIZE_LIMIT 之后补空串，模拟迁移后的建表结构。
                    # 每条数据行为 ...SIZE_LIMIT,'',<换行/逗号/分号>，行尾是 note 空串 "''"，
                    # 用 ", '' )" 模式把 note 空串后追加 ", ''" 形成 ORIGINAL_LANGUAGE。
                    stmt = stmt.replace(col_list, col_list_new, 1).replace("\r\n", "\n")
                    stmt = re.sub(r",''\)", ",'','')", stmt)

...
import log
from app.infrastructure.temp.manager import temp_manager
...
class TempCleanup:
    ...
    def do_cleanup():
        ...
        """
        try:
            log.info("[TempCleanup]开始执行临时文件清理...")
            ...
            deleted_count = temp_manager.cleanup_old_files(
                max_age_hours=TempCleanup.DEFAULT_MAX_AGE_HOURS,
                exclude_patterns=TempCleanup.EXCLUDE_PATTERNS,
            )
            ...
            if deleted_count > 0:
                log.info(f"[TempCleanup]清理完成：删除 {deleted_count} 个文件，大小从 {size_before} 变为 {size_after}")
            else:
                log.debug(f"[TempCleanup]没有需要清理的临时文件，当前大小: {size_after}")

        except Exception as e:
            log.error(f"[TempCleanup]执行清理任务出错: {str(e)}")
    ...
    def get_temp_info():
        ...
        try:
            ...
            log.error(f"[TempCleanup]获取临时目录信息失败: {str(e)}")

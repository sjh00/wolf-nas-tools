...
def check_userrss_task(
    ...
    user: UserContext = Depends(require_permission("rss:manage")),
    svc: UserRssService = Depends(get_user_rss_service),
):
    try:
        svc.check_tasks(taskids=req.ids, flag=req.flag or "", user=user)
...
def delete_userrss_task(
    ...
    svc.delete_task(req.id, user=user)
    return success()
...
def list_rss_parsers(
    ...
    user: UserContext = Depends(require_any_permission("rss:view", "rss:manage")),
    svc: UserRssService = Depends(get_user_rss_service),
):
...
def get_userrss_task(
    ...
    return success(data={"detail": svc.get_task(req.id, user=user)})
...
def list_rss_tasks(
    ...
    return success(data=svc.get_tasks(user=user))
...
def list_rss_articles(
    ...
    dto = svc.get_articles(req.id, user=user)
    if dto.articles:
        return success(
...
def list_rss_history(
    ...
    dto = svc.get_history(req.id, user=user)
    if dto.downloads:
...
def rss_articles_check(
    ...
    if not req.articles:
        return fail(code=ErrorCode.PARAM_VALIDATION_FAILED)
    res = svc.check_articles(taskid=req.taskid, flag=req.flag, articles=req.articles, user=user)
...
def rss_articles_download(
    ...
    if not req.articles:
        return fail(code=ErrorCode.PARAM_VALIDATION_FAILED)
    res = svc.download_articles(taskid=req.taskid, articles=req.articles, user=user)
    return success() if res else fail()
...
def update_userrss_task(
    ...
    dto = svc.update_task(req.data or {}, user=user)

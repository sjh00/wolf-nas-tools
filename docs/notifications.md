...
全局渠道配置（上面「新增消息通知」）用于接收**系统级事件**（站点签到、刷流、系统故障、超级管理员事件）。多用户场景下，每个用户还可以绑定**自己的**渠道身份，用于接收**归属自己**的事件（订阅完成、下载开始/入库）。

入口：**个人设置 → 渠道绑定**（`/profile` → 渠道绑定）。

### 交互渠道（Telegram / 企业微信 / Slack / 飞书）

1. Web 端点击「生成绑定码」，得到 6 位数字码（10 分钟内有效）
2. 在对应 IM 中向机器人发送 `/bind <绑定码>`
3. 绑定成功后，该 IM 账号即与当前系统用户关联
4. 解绑：渠道绑定列表中点击解绑图标；`/unbind` 亦可自助解绑

绑定后可直接在 IM 中搜索、订阅；订阅/下载完成通知会定向推送到该 IM。

!!! warning "未绑定行为"
    交互渠道入站消息**必须完成绑定**才会被处理；未绑定用户发消息会收到绑定指引。这是为了避免多人共用机器人时无法识别身份。

### 推送渠道（Bark / Ntfy / Gotify / Server酱 / PushPlus / PushDeer / 钉钉 / Chanify）

这类渠道只能被系统推送、无法接收消息，因此**无法用绑定码验证**。在 Web 端「渠道绑定 → 推送渠道」直接登记你自己的推送 Key 即可：

| 渠道 | Key 获取位置 |
|------|-------------|
| Bark | Bark App 首页地址中的 Key（`https://api.day.app/<KEY>/`） |
| Ntfy | 你的 ntfy topic 名称（自建或 ntfy.sh） |
| Gotify | Gotify 后台创建应用的 Token |
| Server酱 | Server酱官网登录后的 SendKey |
| PushPlus | PushPlus 个人中心的 token |
| PushDeer | PushDeer App 内的 PushKey |
| 钉钉 | 群机器人 webhook 的 access_token（个人主动推送受平台 session 有效期限制，可能不稳定） |
| Chanify | Chanify App 的 Token |

### 通知路由规则

| 事件类型 | 推送目标 |
|----------|---------|
| 归属用户事件（订阅完成、下载开始/失败、入库完成） | 该用户的 Web 消息 + 已绑定的渠道（定向单发） |
| 系统事件（站点签到、刷流、系统故障等） | 全局渠道配置 |

未绑定任何渠道的用户仍会收到 Web 消息（站内通知），不影响使用。

### 常见问题

- **收不到通知**：确认「渠道绑定」列表中有该渠道；检查全局渠道配置是否停用；推送渠道 Key 是否失效。
- **Telegram 发消息无响应**：确认已发送 `/bind` 完成绑定；机器人 Token / 代理配置是否正确。
- **管理员初始绑定**：存量 Telegram `admin_ids` 会在启动时自动绑定到第一个超级管理员；其他交互渠道需管理员本人手动绑定一次。

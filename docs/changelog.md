...
- 日志：`log.debug()` 与标准库日志拦截在写入 Web 日志缓冲区（`LOG_BUFFER`）前按配置级别过滤，修复「级别设为 info 后 Web 日志页仍显示 DEBUG」；file / stderr sink 行为不变
...
- 恢复 v4.20.10 中被误删的迁移 `8599a96d1abd`。已应用过 v4.20.8/v4.20.9 的库其 `alembic_version` 记录该 revision，文件缺失会导致 `Can't locate revision`，启动迁移失败
- `35165c5e40c2` 重新接回 `8599a96d1abd`，迁移链恢复为 `d8e9f0a1b2c4 → 8599a96d1abd → 35165c5e40c2`

## v4.20.10 (2026-09-12)

### 修复

- 新增单个 head 幂等自愈迁移（`35165c5e40c2`）：确保订阅/RSS/下载历史/搜索结果等表 `USER_ID` 与索引存在，修复"库 alembic 版本已到 head 但加列未执行"导致的 `1054/UndefinedColumn`
- 迁移文件合并为单个：删除冗余的 `8599a96d1abd`，将其 `down_revision` 直接接到 `d8e9f0a1b2c4`

## v4.20.9 (2026-09-12)

### 修复

- 媒体类型：TMDB 返回 tv 但缺少 genre 时不再把已推断的 ANIME 降级为 TV，修复动漫剧集误入 TV 库目录
- qBittorrent：403（会话失效/被临时封禁）自动重新登录一次并重试；403/连接类错误限流告警，不再刷屏
- 刷流：种子属性检查优先使用实时站点凭证（cookie/api_key/bearer/ua/headers），修复 CookieCloud 更新后仍用旧/空 cookie 导致详情页抓取为空
...
- 修复 agent 工具配置测试桩缺少 `set_merged` 导致 CI 失败（仅测试更新，无运行时代码变更）

## v4.20.6 (2026-09-11)

### 修复

- 下载/订阅健壮性：单个候选异常不再中断整批；种子属性详情抓取失败降级为"无属性继续"并自动回退下一候选
- 属性鉴权按站点分流：页面详情走站点 cookie（HD杜比等）、API 详情走 api_key（M-Team 等），修复"种子详情页抓取为空 / key無效"
- 目录列表接口统一走存储后端，路径不存在/权限不足/目录被删返回空列表，修复 500
- qBittorrent 离线优雅降级并限流日志，修复 APIConnectionError 刷屏
- 应用关闭期搜索提交静默跳过；站点 5xx/连接类错误降级为 warn，减少系统异常堆栈
- 刮削等结构化配置改为深合并写入，修复 agent/接口提交不完整 payload 时配置键被整体覆盖丢失
- U2（幼儿园）签到补 `_csrf` 并按"已签到"二次校验（选错验证码也计出勤）

## v4.20.5 (2026-09-11)

### 修复

- 恢复 v4.20.4 发布事故中被误清空的 `pyproject.toml` 与 `uv.lock`（该版本已作废）
- U2（幼儿园）自动签到：showup 表单补齐 `_csrf`（原缺失导致 403 "Invalid or expired link"，报"签到接口请求失败"）；提交候选只取 `captcha_*`，补 `Referer/Origin`；验证码答错不影响出勤，改为提交后二次查询页面按"已签到"判定成功

## v4.20.3 (2026-09-11)

### 新增

- 近期下载支持删除单条记录与「清空全部」（权限 `library:manage`，非超管仅限本人/系统记录）

### 修复

- 订阅：修改开始集数时重置持久化的缺失集列表，避免进度被旧列表推回
- Chrome：统一后端浏览器会话指纹画像，修复 user_1 与 default 多实例；实例达上限不跨画像复用、默认上限 2；浏览器并发默认降至 2
- 消息：订阅/下载/入库等归属事件统一用户级定向（Web + 绑定渠道），渠道名兼容中英文（修复微信绑定收不到）；定向消息按渠道模板渲染（修复丢失模板）
- 入库通知：下载/转移历史被清理后按订阅（TMDB+季/电影）反查归属，避免退化为系统级广播
- 站点搜索：过盾通过后再请求搜索链接，挑战页不再被当作结果返回
- 基础设置：允许清空代理等非敏感项（空值保存生效），密码/密钥等敏感字段仍保留原值

## v4.20.2 (2026-09-11)

### 修复

- 文件转移后刮削 NFO 误用含扩展名的文件名，生成 `xxx.mkv.nfo` 导致媒体库不识别并自行生成 `movie.nfo`；现改为使用去扩展名的文件名（蓝光原盘保留目录名）

## v4.20.1 (2026-09-11)

### 修复

- 修复交互渠道（钉钉/飞书/Slack/Synology Chat）回调测试未同步渠道绑定签名导致的 CI 用例失败（仅测试更新，无运行时代码变更）

## v4.20.0 (2026-09-11)

### 新增

- **多用户数据权限与站点访问控制**（ADR-021）：三层权限模型（功能权限 + 数据归属行级隔离 + 站点资源授权）
  - 订阅、自定义 RSS、搜索结果、下载历史按用户隔离，A/B 用户互不可见，`superadmin` 全量可见
  - 角色级 + 用户级站点授权（搜索/RSS 用途粒度），仅授权站点可用于搜索/订阅，站点统计数据按可见站点裁剪
  - 权限快照服务端缓存（授权变更即时生效，不信任 JWT 副本）；IM 渠道身份绑定（`/bind` 绑定码、未绑定拒绝交互）；通知按归属用户定向推送
  - 新增权限码 `site:assign` / `user:self` / `download:create` / `transfer:view`；`POST /search` 改用 `search:execute`
  - 默认「普通用户」角色移除站点管理与服务面板权限；转移历史/未识别列表收敛为管理员可见

### 修复

- 环境变量数据库配置不再写回 `data/config.yaml`（避免一次性命令/测试的临时配置覆盖真实配置）
- 订阅下载：候选失败后自动回退到下一站点，失败候选打标并写入负缓存（不可重试 6 小时），按链接单飞锁 + 轮内种子解析缓存，修复并发/多阶段重复取链导致的站点当日下载次数被快速耗尽
- 站点业务限额错误识别（如 M-Team「相同種子當天最多下載10次」）不再做无效重试，失败通知去重
- 订阅调度：RSS 同一季缺失条目按季覆盖去重；队列/RSS/主动搜索开始执行即记录运行时间，异常不再每 5 分钟紧循环重试
- 多用户：订阅下载全链路透传 `user_id`，下载记录与通知归属订阅用户而非系统
- qBittorrent 5.2 兼容：`torrents_add` 返回 `TorrentsAddedMetadata` 时按 `success_count` 判定成功（兼容旧版 `Ok` 字符串）；添加失败通知携带下载器原始返回原因
- 下载失败通知标题补齐季集，错误信息不再重复拼接；默认全局失败模板不再输出签名下载链接

### 优化

- 消息模板统一为逐字段换行；下载失败通知展示站点/种子/原因
- 站点数据统计改为「共 N 站 + 每站一行」，中英混排站点名按显示宽度对齐，缺失指标用 `-` 占位
...
- RSS 订阅下载：同一轮中同一下载链接（enclosure）重复候选先去重，避免对同一种子反复解析/下载消耗站点当日下载次数并重复推送失败通知

## v4.19.5 (2026-09-09)

### 修复

- 自定义识别词/词组：TMDB 缺失日期/季数等空值不再崩溃或写入 null，入库前全部字段归一（PostgreSQL NOT NULL 兼容）
- 自定义识别词保存：`group_type` 统一按字符串契约传输

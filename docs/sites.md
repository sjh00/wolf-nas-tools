...
2. **请求头配置**
   ...
   ```
   ⚠️ 注意：Passkey 需要包含 `Bearer ` 前缀

3. **签到配置（可选）**
   - 如果需要自动签到功能，需要在请求头中额外添加签到专用 token：
   ```json
   {"x-sign-token": "你的签到令牌"}
   ```
...
### 高级配置
- **RSS订阅地址**
  ℹ️ 用于[电影订阅](/rss/movie)/[电视剧订阅](/rss/tv)和刷流
  ⚠️ 解析种子详情会增加站点压力
...
数据统计页面位于 [/site/statistics](/site/statistics)。

## 自动刷新

- 在[基础设置 > 服务配置](/system/basic)中配置自动刷新数据周期，打开`站点消息`后将发送当日增量数据情况。
...
刷流规则页面位于 [/brush/rules](/brush/rules)。

刷流规则是模板化的规则配置，包含选种规则、删种规则和停种规则，供刷流任务引用。

## 新增刷流规则

- **规则名称**：规则模板名称，便于在任务中选择引用。
- **选种规则**：配置筛选条件，如站点、促销类型、体积范围、做种人数等。
- **删种规则**：配置自动删除条件，如分享率、做种时间、上传量/下载量阈值等。
- **停种规则**：配置暂停条件，如达到指定分享率或做种时间后停止做种。

> 规则本身不会执行，需要在[刷流任务](/brush)中引用后才能生效。
{.is-info}

# 刷流任务

刷流任务页面位于 [/brush](/brush)。
...
- **执行周期**：支持`5位cron`表达式。
- **引用规则**：选择已创建的[刷流规则](/brush/rules)，任务将按照规则中的选种/删种/停种逻辑执行。
...
在 **设置 > 基础设置** 页面顶部，点击「更新站点配置」按钮可手动触发：
- 显示当前版本和远程版本
- 有新版本时按钮变为高亮状态
- 更新后即时生效，无需重启
...
1. 站点配置从镜像内置的 `config/sites/` 解耦
2. 持久化存储在 `/config/sites/` 目录（Docker 卷挂载）
3. `SiteEngine` 优先加载 `/config/sites/`，不存在时回退到内置配置

# 站点资源

站点资源页面位于 [/site/resources](/site/resources)。

- 在[站点维护](/site/list)中维护好站点后，只要被本工具支持，就会在这里显示站点卡片，点击卡片可以显示站点首页的资源。
